...
SELECT total.court, CONCAT(total.firstname,  ' ', total.surname) AS Name
FROM 
    (
...
total
GROUP BY total.Court, total.Firstname, total.Surname
ORDER BY Name
...
FROM 
    (
    SELECT total.Facility, SUM(total.costs) AS Total_Revenue
    FROM 
        (
    ...
    total
    GROUP BY total.Facility
    )
answer
WHERE (answer.Total_Revenue < 1000)
